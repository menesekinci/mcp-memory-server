...
- TS/JS caller precision is the strongest path, including import/barrel resolution, selective TypeScript compiler API symbol resolution, simple instance method calls, and TSX/JSX component usage. Python is supported with symbol discovery, async functions, same-file calls, relative/module imports, dotted module calls, package re-exports, `self.method()`, same-file and imported-base inherited `self.method()`, `super().method()`, and local or `self.attr` constructor-assigned instance method calls. Go support has started with function/type/method indexing, same-package calls, `go.mod`, local `replace`, `vendor`, and `go.work` module import calls, local constructor-assigned instance method calls, receiver method calls, embedded struct promoted method calls, simple interface parameter dispatch, generated Go file exclusion, and Go build-constraint exclusion.
...
- Go language depth now covers local `go.mod replace` import resolution and simple interface parameter dispatch as lower-confidence concrete method candidates, with integration and benchmark coverage before deeper Go build-system support.
- Go language depth now covers vendor import resolution and explicit `ignore` build-constraint exclusion, with integration and benchmark coverage to reduce noisy Go indexing in real repositories.
- Go language depth now uses a shared build-constraint evaluator for `//go:build`, legacy `// +build`, GOOS/GOARCH filename suffixes, and optional custom tags so indexer and caller-target lookup agree on which Go files are active.
- Doctor/setup polish now counts `.go` files as supported sources and reports the Go build context (`GOOS`, `GOARCH`, `CGO_ENABLED`, and `MCP_MEMORY_GO_BUILD_TAGS`) so custom-tag indexing issues are easier to diagnose.
