...
| `language_depth_js_python_callers` | JavaScript symbol extraction plus Python same-file, async, from-import, re-export, module-import, inheritance, `super()`, instance-method caller extraction, and early Go caller extraction with replace/vendor/workspace import, interface-dispatch, generated-file, and build-expression checks. |
...
- Early Go checks for function/type/method indexing, same-package calls, module import calls, local `replace` imports, vendor imports, `go.work` workspace imports, local constructor-assigned instance method calls, receiver method calls, embedded struct promoted method calls, simple interface parameter dispatch, generated Go file exclusion, explicit `ignore` build-constraint exclusion, and false `//go:build` expression exclusion.
